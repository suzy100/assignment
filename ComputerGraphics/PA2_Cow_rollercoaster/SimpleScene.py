...
def display():
    global cameraIndex, cow2wld, cows, idx, stop, animTime, animStartTime, called, first, isDrag
    ...
    if stop:
        animTime=glfw.get_time()-animStartTime
        if idx==0:
            drawCow(spline(animTime, cows[5], cows[0], cows[1], cows[2]), False)
        elif idx==5:
            drawCow(spline(animTime, cows[4], cows[5], cows[0], cows[1]), False)
            idx=-1
            called+=1
        elif idx==4:
            drawCow(spline(animTime, cows[3], cows[4], cows[5], cows[0]), False)
        else:
            drawCow(spline(animTime, cows[idx-1], cows[idx], cows[idx+1], cows[idx+2]), False)
        
        if animTime>1:
            idx+=1
            animStartTime=glfw.get_time()
            # should initialize when cow rode third round
            if called==3:
                stop=0
                cow2wld = cows[0]
                cows=[]
...
# get point using Catmull-rom spline
def spline(t, p0, p1, p2, p3):
    p = 0.5*(2*p1 + (-p0+p2)*t + (2*p0-5*p1+4*p2-p3)*pow(t,2) + (-p0+3*p1-3*p2+p3)*pow(t,3))
    return p
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, flag_h, flag_v, cow2wld, cursorOnCowBoundingBox, cows, cur_x, stop, called, animStartTime, first
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            if isDrag==H_DRAG and stop:
                isDrag=0
            else:
                isDrag=V_DRAG
                cur_x = x           # save x to fix value of x in vertical drag mode.
            ...
            # if first click >> just picking cow
            if first:
                first=0
            else:
                # save cows' position to calculate cow's route.
                if len(cows) < 6:
                    cows.append(cow2wld)
                    print("inputState:%d\n" % len(cows))
                    if len(cows)==6:
...
def onMouseDrag(window, x, y):
    global isDrag,cursorOnCowBoundingBox, pickInfo, cow2wld, c_vertical, c_horizon, flag_h, flag_v, cur_x
    if isDrag:
        ...
        if  isDrag==V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                # put cur_x >> only move vertical
                ray=screenCoordToRay(window, cur_x, y);
                pp=pickInfo;
                if flag_h:
                    pp.cowPickPosition = c_horizon
